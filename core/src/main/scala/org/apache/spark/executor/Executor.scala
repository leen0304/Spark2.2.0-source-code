...
private[spark] class Executor(
  ...
  class TaskRunner(
    ...
    override def run(): Unit = {
      ...
      try {
        ...
        // 功能比较多。反射加载类创建对象；可以指定上下文环境，进行加载、获取
        task = ser.deserialize[Task[Any]](
          taskDescription.serializedTask, Thread.currentThread.getContextClassLoader)
        task.localProperties = taskDescription.properties
        task.setTaskMemoryManager(taskMemoryManager)

        // 如果这个任务在我们反序列化之前已经被杀死了，那我们现在就退出吧
        // Otherwise,继续执行任务。
   ...
   *
   * 如果我们从sparkContext接收到一系列新的files和jars，下载缺失的依赖
   * 同时也添加类加载器中的新jar
   */
  private def updateDependencies(newFiles: Map[String, Long], newJars: Map[String, Long]) {
    ...
    /**
     * Java的synchronized进行多线程并发访问的同步
     * task实际上是一Java线程的方式在 CoarseGrainedExecutorBackend进程中并发执行的
     * 在执行的过程中，需要访问一些共享的资源，例如：配置文件
     * 那么可能会出现多线程并发访问安全问题
     * 所以，spark选择多线程并发访问的同步；
     * why???????
     * 在里边访问了一些共享的资源
     */
    synchronized {
