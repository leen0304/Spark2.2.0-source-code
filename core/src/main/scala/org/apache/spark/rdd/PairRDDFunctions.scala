...
class PairRDDFunctions[K, V](self: RDD[(K, V)])
   ...
   *
   * 用一种结合和交换的reduce函数 合并每一个key的value
   * 【【自带Shuffle优化】】
   * 在将结果发送给reducer执行之前，也将在每个mapper上执行本地合并，就像MapReduce的combiner
   */
   ...
   * Merge the values for each key using an associative and commutative reduce function. This will
   * also perform the merging locally on each mapper before sending results to a reducer, similarly
   * to a "combiner" in MapReduce. Output will be hash-partitioned with the existing partitioner parallelism level.
   * 输出将使用 自己默认的8个 进行哈希分区。
   *
   * spark.default.parallelism: 控制Spark中的分布式shuffle过程默认使用的task数量，默认为8个。
   * 如果不做调整，数据量大时，就容易运行时间很长，甚至是出Exception，因为8个task无法handle那么多的数据。
   * 注意这个值也不是说设置得越大越好。
   */
