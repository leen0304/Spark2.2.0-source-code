...
abstract class RDD[T: ClassTag](
  ...
  /**
   * leen
   * rdd的内部方法；将从缓存读取，如果适用的话，或者计算它。
   * 之后：返回计算之后RDD的计算结果数据
   */
  final def iterator(split: Partition, context: TaskContext): Iterator[T] = {
    // 如果存储级别不为NONE，也就是说我们持久化过RDD
    // 那么就不用从父类RDD中执行算子，计算新的RDD的partition
    // 则优先使用CacheManager，去获取持久化的数据
    if (storageLevel != StorageLevel.NONE) {
  ...
  private[spark] def getOrCompute(partition: Partition, context: TaskContext): Iterator[T] = {
    ...
    // 这个方法调用了executor，所以我们需要调用SparkEnv。而不是sc.env。
   ...
   * 对RDD中的所有元素应用一个函数处理，之后返回一个新的RDD
   */
   ...
   *  首先对RDD中的所有元素应用一个函数处理，之后再压平结果集 ，最后返回一个新的RDD
   */
   ...
   * 返回一个新的RDD，它减少到“num分区”个分区。
   *
   * This results in a narrow dependency, e.g.
   * 这是一个窄依赖操作；
   * if you go from 1000 partitions to 100 partitions, there will not be a shuffle,
   * 如果从1000个分区合并成100各分区，将不会有Shuffle操作
   * instead each of the 100 new partitions will claim 10 of the current partitions.
   * 100个新分区中的每一个将占据当前分区的10个。
   * If a larger number of partitions is requested, it will stay at the current number of partitions.
   * 如果要求更多的分区个数，将保持为当下的分区个数。
   *
   * However, if you're doing a drastic coalesce, e.g. to numPartitions = 1,
   * 但是，当我们进行一个剧烈的合并，设置numPartitions = 1
   * this may result in your computation taking place on fewer nodes than you like (e.g. one node in the case of numPartitions = 1).
   * 这可能导致你的计算比你想要的节点少，当numPartitions = 1时，只会在一个节点上合并
   * To avoid this,you can pass shuffle = true. This will add a shuffle step,
 ...
 * 定义隐式转换函数，对ＲＤＤ操作提供额外的功能
 * For example, [[RDD.rddToPairRDDFunctions]] converts an RDD into a [[PairRDDFunctions]] for key-value-pair RDDs,
 * and enabling extra functionalities such as `PairRDDFunctions.reduceByKey`.
 * 例如，RDD.rddToPairRDDFunctions 转变一个RDD为PairRDDFunctions， key-value-pair RDDs
 * 例如PairRDDFunctions.reduceByKey
 */
...
object RDD {
  ...
  // `import SparkContext._` to enable them. Now we move them here to make the compiler find them automatically.
  // However, we still keep the old functions in SparkContext for backward compatibility and forward to the following functions directly.
  // 在Spark1.3之前，必须手动导入 “import SparkContext._”，现在可以直接自动获取
