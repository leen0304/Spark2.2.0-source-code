 ...
 *
 * 管理每个节点(驱动程序和执行者)的管理器，它提供了用于放置的接口
 * 在本地和远程检索多个块(内存、磁盘和堆外)
 *
...
private[spark] class BlockManager(
    ...
    blockInfoManager.lockForReading(blockId) match {
      ...
      case Some(info) =>
        ...
        if (level.useMemory && memoryStore.contains(blockId)) {
          ...
          // We need to capture the current taskId in case
          // the iterator completion is triggered from a different thread
          // which does not have TaskContext set;
          // see SPARK-18406 for discussion.
   ...
   * and does not acquire any locks if the block was fetched from a remote block manager.
   * The read lock will automatically be freed once the result's `data` iterator is fully consumed.
   * 如果块存储在本地，就可以在块上获得读锁；
   * 如果块是从远程块管理器获取的，则不会获得任何锁。
   * 一旦结果的“数据”迭代器被完全耗尽，读锁将自动释放。
   */
  ...
  def getOrElseUpdate[T](
    ...
    // If it's present, then we don't need to go through the local-get-or-put path.
    /**
     *
     * 尝试从本地或远程存储中读取该块。
     */
  ...
  private def doPutIterator[T](
    ...
    doPut(blockId, level, classTag, tellMaster = tellMaster, keepReadLock = keepReadLock) { info =>
      ...
      if (level.useMemory) {
        // Put it in memory first, even if it also has useDisk set to true;
        // 把它放在内存中，即使它也有正确的磁盘设置;
        // We will drop it to disk later if the memory store can't hold it.
        // 如果内存存储无法保存，我们将稍后将其删除到磁盘
        if (level.deserialized) {
