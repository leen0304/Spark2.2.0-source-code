...
private[spark] class MemoryStore(
   ...
   * 尝试将块清除，以释放给定的空间以存储特定的块。
   * Can fail if either the block is bigger than our memory or it would require replacing another block from the same RDD
   * 如果一个块大于我们的内存，或者它需要从同一个RDD中替换另一个块，那么它会失败吗
   * (which leads to a wasteful cyclic replacement pattern for RDDs that don't fit into memory that we want to avoid).
   * 这导致了一种浪费的循环替换模式，用于那些不符合我们想要避免的内存的RDDs
   *
