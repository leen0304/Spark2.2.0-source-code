...
class SparkContext(config: SparkConf) extends Logging {
   ...
   * If you plan to directly cache, sort, or aggregate Hadoop writable objects, you should first
   * copy them using a `map` function.
   * @param path 目录到输入数据文件，路径可以是逗号分隔的路径作为输入的列表
   * @param inputFormatClass 读取数据的存储格式
   * @param keyClass `Class` of the key associated with the `inputFormatClass` parameter
   * @param valueClass `Class` of the value associated with the `inputFormatClass` parameter
   * @param minPartitions 建议产生的RDD的最小分区数
   * @return (K,V)元组的RDD
  ...
  /**
   * Broadcast a read-only variable to the cluster, returning a object for reading it in distributed functions.
   * 向集群广播一个只读变量，返回在分布式函数中读取它的对象。
   * [[org.apache.spark.broadcast.Broadcast]]
   * The variable will be sent to each cluster only once.
   * 变量将只发送到每个集群一次。
   * @param value value to broadcast to the Spark nodes
   * @return `Broadcast` object, a read-only variable cached on each machine 在每台机器上缓存一个只读变量
   */
   ...
   */
  /**
   * 在RDD中分区集合上运行一个函数，并将结果作为一个数组返回。
   * 另外，针对每个分区运行的函数使用“TaskContext”参数。
   * @param rdd
   * @param func
   * @param partitions
   * @tparam T
   * @tparam U
   * @return 包含工作结果的内存中的集合(每个集合元素都将包含一个分区的结果)
   */
  def runJob[T, U: ClassTag](
      rdd: RDD[T],
      func: (TaskContext, Iterator[T]) => U,
   ...
   * @param rdd target RDD to run tasks on
   * @param func a function to run on each partition of the RDD
   * @param partitions set of partitions to run on;
   * some jobs may not want to compute on all partitions of the target RDD, e.g. for operations like `first()`
   *
   * @return in-memory collection with a result of the job (each collection element will contain
   * a result from one partition)
   */
  ...
  /**
   * 在RDD中在给定的分区上运行一个函数，并将结果作为一个数组返回
   * @param rdd
   * @param func
   * @param partitions 要运行的分区set集合,有些作业可能不想在目标RDD的所有分区上进行计算，比如像“first()”这样的操作。
   * @return job的结果集合，每一个集合元素包含一个分区的结果；
   */
  def runJob[T, U: ClassTag](
      rdd: RDD[T],
      func: Iterator[T] => U,
   ...
   * @return  在内存中的集合 含有job的结果，每个集合的元素包含一个分区的结果
   */
