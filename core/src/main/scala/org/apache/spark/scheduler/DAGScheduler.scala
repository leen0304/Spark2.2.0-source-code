...
class DAGScheduler(
  ...
  private def submitMissingTasks(stage: Stage, jobId: Int) {
    ...
    // Use the scheduling pool, job group, description, etc.
    // from an ActiveJob associated with this Stage
    val properties = jobIdToActiveJob(jobId).properties

    runningStages += stage
    // SparkListenerStageSubmitted should be posted before testing whether tasks are serializable.
    // If tasks are not serializable, a SparkListenerStageCompleted event will be posted,
    // which should always come after a corresponding SparkListenerStageSubmitted event.
    stage match {
    ...
    if (tasks.size > 0) {
      ...
      // Because we posted SparkListenerStageSubmitted earlier,
      // we should mark the stage as completed here in case there are no tasks to run
   ...
   *
   * 递归实现方法找到 最佳位置 PreferredLocs.
   *
   * This method is thread-safe because it only accesses DAGScheduler state through thread-safe methods (getCacheLocs());
   * please be careful when modifying this method,
   * because any new DAGScheduler state accessed by it may require additional synchronization.
   */
  private def getPreferredLocsInternal(
    ...
    /**
     * 【1】 如果这个Partition已经被缓存，则返回缓存的位置信息
     */
    ...
    /**
     * 【2】如果RDD设定了checkPoint 那么返回checkpoint的位置
     */
    ...
    // If the RDD has narrow dependencies,
    // pick the first partition of the first narrow dependency that has any placement preferences.
    // Ideally we would choose based on transfer sizes,
    // but this will do for now.
    /**
     * 【3】 递归调用自己，寻找RDD的父RDD，看看对应的Partition是否缓存 或者 checkPoint
     */
    ...
    /**
     * 【4】 如果这个Stage，从finalRDD到最开始的RDD，Partition都没有被缓存或者 checkPoint
     * 那么，task的最佳位置，就是Nil
     */
    Nil
  }
