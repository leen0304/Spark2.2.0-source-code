...
/**
 * 在TaskSchedulerImpl中，对一个单独的TaskSet的tasks进行调度；
 * 这个类负责跟踪每一个task任务，当tasks失败的时候重试task,（直到超过重试次数限制）
 * 通过延迟调度，围着个TaskSet处理本地化调度机制
 * 它的只要接口是 resourceOffer，resourceOffer询问TaskSet是否要在一个节点上运行一个task,
 * 并且更新状态，来知道其中的一个task状态发生了改变。
 */
private[spark] class TaskSetManager(
   ...
   *
   * NOTE: this function is either called with a maxLocality
   * which would be adjusted by delay scheduling algorithm
   * or it will be with a special NO_PREF locality which will be not modified
   *
  ...
  /**
   * leen
   * 这个方法：会去判断这个executor在这个本地化级别之前的等待时间是多少
   * 如果说，本地化级别的等待时间在一定范围内，
   * 那么认为task使用这种本地化级别可以在executor上启动
   */
