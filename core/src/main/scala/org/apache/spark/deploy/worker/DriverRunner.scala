 ...
 * leen
 * 管理Driver，包含当Driver失败的时候重启Driver
 * 这种方式仅仅被用在standalone的集群部署模式
 */
private[deploy] class DriverRunner(
      ...
      override def run() {
        ...
        try {
          shutdownHook = ShutdownHookManager.addShutdownHook { () =>
            ...
            /**
             * 准备Driver的jars并且运行Driver，返回 状态值
             */
          val exitCode = prepareAndRunDriver()

          // 设置最终状态取决于强制死亡和过程退出代码
        ...
        // 通知Worker，Driver的最终状态 ，调用方法 Worker.handleDriverStateChanged()
  ...
  /**
   * leen
   * 下载用户的jar到提供的文件夹中，并返回它们的本地路径
   * @param driverDir
   * @return String  jars本地路径
   */
  private def downloadUserJar(driverDir: File): String = {
    ...
    // 如果在一个节点上 运行多个Worker，则jar已经存在此节点中
    // 如果本地不存在
  ...
  private[worker] def prepareAndRunDriver(): Int = {
    ...
    // TODO: 如果我们增加了提交多个jar的能力，它们也应该在这里添加
    // 创建 ProcessBuilder
